...
- **Scope note:** `writeCustomFields` is currently only wired into `netbox_device_type` Create. ~30 other resources assign `data.CustomFields` directly from `d.GetOk()` without going through any helper — they have the same double-encoding bug but are out of scope for this patch. Follow-up work needed to apply `writeCustomFields` broadly.
