...
- **Tests:** `TestAccNetboxDeviceType_extendedFields`, `TestAccNetboxDeviceType_cf_clear`, `TestAccNetboxDeviceType_cf_unrelatedRegistered`, `TestAccNetboxDeviceType_weightUnitRequiresWeight`
...
- **Files:** `netbox/resource_netbox_available_ip_address.go`, `netbox/resource_netbox_available_ip_address_test.go`
- **Tests:** `TestPickShuffledIP_emptyPool`, `TestPickShuffledIP_singleEntry_usesIt`, `TestPickShuffledIP_full_neverPicksFirst`, `TestPickShuffledIP_full_coversAllNonFirst`, `TestPickShuffledIP_low_staysInBottom20Percent`, `TestPickShuffledIP_low_twoEntries_picksSecond`, `TestPickShuffledIP_low_singleEntry_usesIt`, `TestResourceNetboxAvailableIPAddress_shuffleModeDoesNotForceNew`, `TestAccNetboxAvailableIPAddress_shuffleModeUpdateDoesNotReplace`
...
- **Type:** Bug fix
- **Introduced:** 2026-05 (`v5.3.7`)
- **Files:** `netbox/custom_fields.go`, `netbox/resource_netbox_device_type.go`, `netbox/resource_netbox_ip_range.go`, `netbox/resource_netbox_available_ip_address.go`, `netbox/resource_netbox_ip_address.go`
...
- **What:** Introduces `coerceJSONStringValue` helper and a new `writeCustomFields()` function in `custom_fields.go`. When a custom field value is a string that parses as a JSON object or array, `coerceJSONStringValue` unmarshals it to a native Go type before sending to the API. `writeCustomFields` is used in Create paths; `customFieldsForUpdate` (Update path) also applies coercion. `getCustomFields` is kept as a pure filter/nil-stripper with no coercion so it stays safe for Read paths that don't see JSON-typed CFs. `readCustomFields` is refactored to build on `flattenCustomFields` (eliminating duplication) and then strips empty-string keys; it stringifies any complex value (map/array) returned by NetBox back into a JSON string for the `map(string)` schema. Read paths for every resource whose Update goes through `customFieldsForUpdate` — `netbox_device_type`, `netbox_ip_range`, `netbox_available_ip_address`, `netbox_ip_address` — are migrated to `readCustomFields` so JSON objects from the API round-trip correctly to `jsonencode()`-shaped strings in state.
- **Scope note:** `writeCustomFields` (Create-path coercion) is currently only wired into `netbox_device_type` Create. ~30 other resources assign `data.CustomFields` directly from `d.GetOk()` without going through any helper — they have the same double-encoding bug on Create but are out of scope for this patch. Their Read paths still use `getCustomFields`, which matches their Create behaviour (consistent bug, no type-mismatch error). Follow-up work needed to apply `writeCustomFields` broadly and migrate their Reads to `readCustomFields` in lockstep.
...
#### `primary-mac` — `primary` attribute on `netbox_mac_address` for interface promotion

- **Type:** Feature
- **Introduced:** 2026-08 (`v5.3.9`)
- **Files:** `netbox/resource_netbox_mac_address.go`, `netbox/resource_netbox_mac_address_test.go`, `netbox/resource_netbox_interface.go`, `netbox/resource_netbox_interface_test.go`
- **Tests:** `TestAccNetboxMACAddress_primaryVM`, `TestAccNetboxMACAddress_primaryDevice`, `TestAccNetboxInterface_macAddressExplicitNoDrift`, `TestAccNetboxInterface_macAddressServerSideNoDrift`
- **Why:** NetBox 4.2+ supports a `primary_mac_address` field on interfaces (both device and VM). The upstream provider has no way to set this, meaning allocated MAC addresses can't be promoted to primary via Terraform.
- **What:** Adds an optional `primary` boolean attribute (default `false`) to `netbox_mac_address`. When `true`, Create PATCHes the assigned interface to set `primary_mac_address` to this MAC's ID. Read checks the interface's current `primary_mac_address` to keep state in sync. Update handles toggling, plus correctly clears the old interface and sets the new one when the MAC is reassigned to a different interface. Delete clears `primary_mac_address` before destroying the MAC. The PATCH includes the interface's existing tags to satisfy NetBox custom validation rules. Also marks `netbox_interface.mac_address` as `Computed: true` to prevent drift when NetBox populates `mac_address` server-side due to primary MAC promotion.
- **Upstream candidate:** Yes — clean candidate, no go-netbox model changes needed for this specific attribute (uses body-override PATCH). The `Computed` change on `netbox_interface.mac_address` is also upstreamable. Currently parked per user direction.
